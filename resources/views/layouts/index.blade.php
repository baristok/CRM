...
<body>
                ...
                <div class="navbar-header">
                    <div class="d-flex">
                        ...
                        <form class="app-search d-none d-md-block">
                            <div class="position-relative">
                                <input type="text" class="form-control" placeholder="{{ __('layout.search') }}"
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="dropdown ms-1 topbar-head-dropdown header-item">
                            <button type="button" class="btn btn-icon btn-topbar btn-ghost-secondary rounded-circle"
                                ...
                                <img id="header-lang-img"
                                    src="{{ asset('assets/images/flags/' . (session('locale') == 'en' ? 'us' : 'tr') . '.svg') }}"
                                    alt="Header Language" height="20" class="rounded">
                            </button>
                            <div class="dropdown-menu dropdown-menu-end">

                                <!-- item-->
                                <a href="{{ route('change.language', 'tr') }}"
                                    class="dropdown-item notify-item language" data-lang="tr" title="Türkçe">
                                    <img src="{{ asset('assets/images/flags/tr.svg') }}" alt="user-image"
                        ...
                        <div class="dropdown ms-sm-3 header-item topbar-user">
                            ...
                            <div class="dropdown-menu dropdown-menu-end">
                                <!-- item-->
                                <h6 class="dropdown-header">{{ __('layout.welcome') }} {{ Auth::user()->name }}!</h6>
                                <a class="dropdown-item" href="pages-profile.html"><i
                                        class="mdi mdi-account-circle text-muted fs-16 align-middle me-1"></i> <span
                                        class="align-middle">{{ __('layout.profile') }}</span></a>
                                <a class="dropdown-item" href="apps-chat.html"><i
                                        class="mdi mdi-message-text-outline text-muted fs-16 align-middle me-1"></i>
                                    <span class="align-middle">{{ __('layout.messages') }}</span></a>
                                <a class="dropdown-item" href="apps-tasks-kanban.html"><i
                                        class="mdi mdi-calendar-check-outline text-muted fs-16 align-middle me-1"></i>
                                    <span class="align-middle">{{ __('layout.taskboard') }}</span></a>
                                <a class="dropdown-item" href="pages-faqs.html"><i
                                        class="mdi mdi-lifebuoy text-muted fs-16 align-middle me-1"></i> <span
                                        class="align-middle">{{ __('layout.help') }}</span></a>
                                <div class="dropdown-divider"></div>
                                <a class="dropdown-item" href="pages-profile-settings.html"><span
                                        class="badge bg-success-subtle text-success mt-1 float-end">New</span><i
                                        class="mdi mdi-cog-outline text-muted fs-16 align-middle me-1"></i> <span
                                        class="align-middle">{{ __('layout.settings') }}</span></a>
                                <a class="dropdown-item" href="{{ route('logout') }}"><i
                                        class="mdi mdi-logout text-muted fs-16 align-middle me-1"></i> <span
                                        class="align-middle" data-key="t-logout">{{ __('layout.logout') }}</span></a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="container-fluid">
                    ...
                    <ul class="navbar-nav" id="navbar-nav">
                        ...
                        <li class="nav-item">
                            ...
                            @php
                                ...
                                foreach (app('menu')->getItems() as $item) {
                                    if (
                                        ...
                                        request()->is(trim(parse_url($item['url'], PHP_URL_PATH), '/') . '/*')
                                    ) {
                                        $isAppsMenuActive = true;
                                        break;
                                    }
                                    
                                    // Child menüleri de kontrol et
                                    if (isset($item['child']) && is_array($item['child'])) {
                                        foreach ($item['child'] as $child) {
                                            if (
                                                request()->url() == $child['url'] ||
                                                request()->is(trim(parse_url($child['url'], PHP_URL_PATH), '/') . '/*')
                                            ) {
                                                $isAppsMenuActive = true;
                                                break 2;
                                            }
                            ...
                            @endphp
                            <div class="collapse menu-dropdown {{ $isAppsMenuActive ? 'show' : '' }}"
                                id="sidebarApps">
                                <ul class="nav nav-sm flex-column">
                                    @foreach (app('menu')->getItems() as $item)
                                        <li class="nav-item @if (request()->url() == $item['url'] ) active @endif">
                                            @if (isset($item['child']) && is_array($item['child']) && count($item['child']) > 0)
                                                ...
                                                @endphp
                                                <a class="nav-link menu-link" href="#sidebar{{ str_replace('.', '_', $item['title']) }}" data-bs-toggle="collapse"
                                                    role="button" aria-expanded="false" aria-controls="sidebar{{ str_replace('.', '_', $item['title']) }}">
                                                    <i class="{{ $item['icon'] }}"></i>
                                                    <span data-key="t-{{ strtolower($item['title']) }}">{{ __($item['title']) }}</span>
                                                </a>
                                                <div class="collapse menu-dropdown {{ $isChildActive ? 'show' : '' }}" id="sidebar{{ str_replace('.', '_', $item['title']) }}">
                                                    <ul class="nav nav-sm flex-column">
                                                        @foreach ($item['child'] as $child)
                                                            <li class="nav-item @if (request()->url() == $child['url']) active @endif">
                                                                <a href="{{ $child['url'] }}" class="nav-link @if (request()->url() == $child['url']) active @endif">
                                                                    <i class="{{ $child['icon'] }}"></i>
                                                                    <span data-key="t-{{ strtolower($child['title']) }}">{{ __($child['title']) }}</span>
                                                                </a>
                                                            </li>
                                                        @endforeach
                                                    </ul>
                                            ...
                                            @else
                                                <a href="{{ $item['url'] }}"
                                                    class="nav-link @if (request()->url() == $item['url']) active @endif">
                                                    <i class="{{ $item['icon'] }}"></i>
                                                    <span
                                                        data-key="t-{{ strtolower($item['title']) }}">{{ __($item['title']) }}</span>
                                                </a>
                                            @endif
                                        </li>
                                    @endforeach
                ...
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-sm-6">
                            ...
                            </script> © {{ __('layout.copyright') }}
                        </div>
                        <div class="col-sm-6">
                            <div class="text-sm-end d-none d-sm-block">
                                <div class="footer-social-links">
                                    <a href="https://github.com/baristok" target="_blank" class="text-reset me-2"
                                        title="GitHub">
                                        <i class="mdi mdi-github fs-16"></i>
                                    </a>
                                    <a href="https://linkedin.com/in/baristok" target="_blank"
                                        class="text-reset me-2" title="LinkedIn">
                                        <i class="mdi mdi-linkedin fs-16"></i>
                                    </a>
                                    <a href="mailto:info@baristok.com.tr" class="text-reset" title="E-posta">
                                        <i class="mdi mdi-email-outline fs-16"></i>
                                    </a>
                                </div>
                            </div>
                        </div>
    ...
    <!-- Theme Settings -->
    @include('components.layouts.themesettings')
